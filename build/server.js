...
mongoose_1.default
  .connect(config_js_1.config.mongo.url, { retryWrites: true, w: "majority" })
  .then(() => {
    Logging_js_1.default.info("Mongo connected successfully.");
    StartServer();
  })
  .catch((error) => Logging_js_1.default.error(error));
...
const StartServer = () => {
  ...
  router.use((req, res, next) => {
    ...
    Logging_js_1.default.error(error);
    res.status(404).json({
